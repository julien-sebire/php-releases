...
$d0 = new DateTime('1995-01-01');
$table = [];
$columns = [];
foreach ($dates as $date => $versions) {
	$days = (new DateTime($date))->diff($d0)->format("%a");
	
	$versionsInColumns = [];
	foreach ($versions as $version) {
		$mainVersion = minorVersion($version);
		if (!isset($columns[$mainVersion])) {
			$columns[$mainVersion] = count($columns);
		}
		$column = $columns[$mainVersion];
...
.key {position:relative;margin-bottom:15px;}
div {font-size:10px;}
.key div {display:inline-block;position:relative;width:auto;padding:2px;}
.eol {background-color:#fdd;}
.security {background-color:#ffd;}
.active {background-color:#dfd;}
.future {background-color:#ddf;}
.table {position:relative;}
.year {position:absolute;left:0;height:365px;width:100%;}
.year.odd {background-color:#ddd;}
.version {position:absolute;width:70px;text-align:center;font-weight:bold;padding-top:2px;border-radius:5px;box-shadow:0 0 3px #333;}
.release {position:absolute;width:70px;text-align:center;}
</style>

<h1>PHP release versions</h1>

<div class="key">
	<div class="version eol">End of life</div>
	<div class="version security">Security fixes only</div>
	<div class="version active">Active support</div>
	<div class="version future">Future version</div>
</div>
...
for ($i = 1995; $i <= $thisYear; $i++) {
		$str .= sprintf(
			'<div class="year%s" style="top:%dpx;">%d</div>' . "\n",
			$i % 2 ? ' odd' : '',
			(new DateTime($i . '-01-01'))->diff($d0)->format("%a"),
			$i
		);
}
...
$bars = [];
$majors = [];
foreach ($table as $days => $day) {
	foreach ($day['versions'] as $column => $version) {
		if (!isset($bars[$column])) {
			$bars[$column] = ['version' => $version, 'from' => $days];
		}
		$bars[$column]['to'] = $days;
	}
}
...
// minor versions
foreach ($bars as $column => $bounds) {
	$version = minorVersion($bounds['version']);
	$str .= sprintf(
		'<div class="version %s" style="top:%dpx;left:%dpx;height:%dpx;" title="%s">%s</div>' . "\n",
		getSupport($version),
		$bounds['from'] - 20,
		$column * 80,
		$bounds['to'] - $bounds['from'] + 35,
		'Version ' . $version,
		$version
	);
}
...
/*
	$str .= sprintf(
		'<div style="position:absolute;top:%dpx;left:0;">%s</div>' . "\n",
		$days,
		$day['date']
	);
	*/
	foreach ($day['versions'] as $column => $version) {
		$str .= sprintf(
			'<div class="release" style="top:%dpx;left:%dpx;" title="%s">%s</div>' . "\n",
			$days,
			$column * 80,
			$version . ' released ' . $day['date'],
			$version
		);
	}
}
...
function minorVersion(string $version): string
{
	$fullVersion = explode('.', $version);
	return $fullVersion[0] . '.' . $fullVersion[1];
}

function getSupport(string $version): string
{
	$support = [
		'7.4' => 'security',
		'8.0' => 'active',
